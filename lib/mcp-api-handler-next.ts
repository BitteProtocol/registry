  ...
  return async function mcpApiHandler(
    ...
    if (url.pathname === "/sse") {
      ...
      try {
        ...
        await redis.subscribe(`requests:${sessionId}`, handleMessage).then(() => {
        ...
        console.log(`Redis connection status: ${redis.isOpen ? 'open' : 'closed'}`);
        throw error; // Re-throw to handle it at a higher level if needed
      }
...
// Create a NextJS adapter for the MCP API handler
export function createNextJsAdapter(mcpApiHandler: ReturnType<typeof initializeMcpApiHandler>) {
  return async function nextJsApiHandler(req: Request) {
    ...
    return new Promise<Response>(async (resolve) => {
      ...
      serverResponse.end = function(this: ServerResponse) {
        ...
        const response = new Response(arguments[0], {
          status: statusCode,
          headers: headers,
        });
        
        console.log("NextJS adapter: Resolving with Response", statusCode);
        resolve(response);
      ...
      // If response.end() was never called, resolve with a 204 response
